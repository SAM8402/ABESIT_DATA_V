...
# Dictionary mapping categories and subcategories to product names
product_names = {
    'Electronics': {
        'Mobile': ['iPhone', 'Samsung Galaxy', 'OnePlus', 'Nokia', 'Google Pixel'],
        'Laptop': ['MacBook', 'Dell Inspiron', 'HP Spectre', 'Lenovo ThinkPad', 'Asus ZenBook']
    },
    'Clothing': {
        'Shirt': ['T-Shirt', 'Dress Shirt', 'Polo Shirt', 'Casual Shirt', 'Formal Shirt']
    },
    'Home': {
        'Sofa': ['Leather Sofa', 'Recliner Sofa', 'Fabric Sofa', 'Sectional Sofa', 'Sleeper Sofa']
    },
    'Beauty': {
        'Makeup': ['Lipstick', 'Foundation', 'Eyeliner', 'Mascara', 'Blush']
    },
    'Sports': {
        'Football': ['Adidas Football', 'Nike Football', 'Puma Football', 'Mitre Football', 'Wilson Football']
    }
}
...
for _ in range(1000):
    ...
    sub_category = random.choice(list(product_names[category].keys()))
    product_name = random.choice(product_names[category][sub_category])  # Select product name based on category and sub_category
